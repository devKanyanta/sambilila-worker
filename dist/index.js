...
async function executeWithRetry(queryFn, maxRetries = 3, operationName = 'database operation') {
    let lastError;
...
async function processJob(job) {
    ...
    try {
        ...
        await executeWithRetry(() => prisma.$executeRaw `
      ...
      `, 3, 'update job status');
        ...
        const flashcardSet = await executeWithRetry(() => prisma.flashcardSet.create({
            data: {
                userId: job.userId,
                title: job.title,
                subject: job.subject,
...
async function pollJobs() {
    ...
    try {
        ...
        const pendingJobs = await executeWithRetry(() => prisma.$queryRaw `
        SELECT 
          id,
          "userId",
          "fileUrl",
...
async function main() {
    ...
    try {
        await executeWithRetry(() => prisma.$queryRaw `SELECT 1`, 3, 'database connection test');
